...
AddEventHandler('MI-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    ...
    if Target ~= nil then
        TriggerClientEvent('MI-phone:client:GetCalled', Target.source, character.phone, CallId, AnonymousCall)
    end
end)
            ...
            if TargetData ~= nil then
                ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `identifier` = '"..SenderData.identifier.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        ...
                        -- Send notification & Update messages for target
                        TriggerClientEvent('MI-phone:client:UpdateMessages', TargetData.source, ChatMessages, SenderCharacter.phone, true)
                    end
                end)
            else
                ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `identifier` = '"..SenderData.identifier.."' AND `number` = '"..ChatNumber.."'", function(Chat)
