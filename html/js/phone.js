...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://MI-phone/ClearRecentAlerts');
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    $.post('http://MI-phone/CallContact', JSON.stringify({
        ContactData: cData,
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== MI.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            $.post('http://MI-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
        ...
        MI.Phone.Notifications.Add("fa fa-phone-alt", MI.Phone.Functions.Lang("PHONE_TITLE"), MI.Phone.Functions.Lang("PHONE_MSG_YOURSELF"), "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://MI-phone/EditContact', JSON.stringify({
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://MI-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://MI-phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://MI-phone/RemoveSuggestion', JSON.stringify({
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('http://MI-phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('http://MI-phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();
    
    $.post('http://MI-phone/CancelOngoingCall');
});
