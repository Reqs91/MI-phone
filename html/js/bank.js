...
$(document).on('click', '.bank-app-account', function(e){
    ...
    MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("BANK_TITLE"), "Bank account No. copied!", "#badc58", 1750);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
        if (amountData >= amount) {
            $.post('http://MI-phone/TransferMoney', JSON.stringify({
                iban: iban,
                amount: amount
            }), function(data){
                if (data.CanTransfer) {
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://MI-phone/PayInvoice', JSON.stringify({
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://MI-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
